...
# ===============================================================
# CLASE COMPRESOR HUFFMAN
# ===============================================================
class CompresorHuffman:
    # Implementación de técnica Huffman
    ...
    def generar_codigos(self, nodo, codigo_actual=""):
        ...
        self.generar_codigos(nodo.left, codigo_actual + "0")
        self.generar_codigos(nodo.right, codigo_actual + "1")
    
    
    def comprimir_a_bytes(self, texto):
        bits_comprimidos, frecuencias = self.comprimir(texto)
        
        # Agregar padding
        padding = (8 - len(bits_comprimidos) % 8) % 8
        bits_comprimidos += '0' * padding
        
        # Convertir a bytes
        bytes_comprimidos = bytearray()
        for i in range(0, len(bits_comprimidos), 8):
            byte = bits_comprimidos[i:i+8]
        ...
        paquete = {
            ...
            'padding': padding,
            'datos': bytes(bytes_comprimidos)
        }
        
        paquete_serializado = pickle.dumps(paquete)
        return base64.b64encode(paquete_serializado).decode('ascii')
    
    def descomprimir_desde_bytes(self, paquete_b64):
        # Desempaqueta y descomprime
        try:
            paquete_serializado = base64.b64decode(paquete_b64.encode('ascii'))
            paquete = pickle.loads(paquete_serializado)
            
            frecuencias = paquete['frecuencias']
            padding = paquete['padding']
            datos = paquete['datos']
            ...
            return self.descomprimir(bits, frecuencias)
        except Exception as e:
            print(f"Error descomprimiendo Huffman: {e}")
            return None
...
class LSBDetector:
    ...
    def calcular_entropia(self, datos):
        ...
        entropia = -np.sum(probabilidades * np.log2(probabilidades + 1e-8))
        return entropia
    ...
    def benchmark_n_vs_tiempo(self, ruta_imagen, 
        ...
        plt.xlabel('n (numero de pixeles)', fontsize=8)
        plt.ylabel('Tiempo (segundos)', fontsize=8)
        ...
        plt.plot(ns, aceleraciones, 'b-o', linewidth=2.5, markersize=8, alpha=0.8)
        ...
        plt.xlabel('n (numero de pixeles)', fontsize=8)
        plt.ylabel('Aceleracion (FB/DyV)', fontsize=8)
        ...
        plt.xlabel('Tamaño de imagen', fontsize=8)
        plt.ylabel('Mejora (%)', fontsize=8)
        ...
        plt.plot(ns, tiempos_fb_promedio, 'ro', markersize=8, label='Datos FB', alpha=0.7)
        plt.plot(ns, ajuste_fb, 'r--', linewidth=2, label=f'Ajuste FB: {parametros_fb[0]:.2e}*n + {parametros_fb[1]:.4f}')
        plt.xlabel('n (pixeles)', fontsize=8)
        plt.ylabel('Tiempo (s)', fontsize=8)
        plt.title('Fuerza Bruta: Complejidad Temporal', fontsize=11)
        ...
        plt.plot(ns, tiempos_dyv_promedio, 'go', markersize=8, label='Datos DyV', alpha=0.7)
        plt.plot(ns, ajuste_dyv, 'g--', linewidth=2, label=f'Ajuste DyV: {parametros_dyv[0]:.2e}*n + {parametros_dyv[1]:.4f}')
        plt.xlabel('n (pixeles)', fontsize=8)
        plt.ylabel('Tiempo (s)', fontsize=8)
        plt.title('Divide y Venceras: Complejidad Temporal', fontsize=11)
        plt.legend(fontsize=8)
        plt.grid(True, alpha=0.3)
...
# ===============================================================
# CLASE ESTEGANOGRAFÍA CON HUFFMAN
# ===============================================================
class EsteganografiaLSB:
    
    @staticmethod
    def comprimir_con_huffman(mensaje):
        """Comprime mensaje usando Huffman"""
        try:
            compresor = CompresorHuffman()
            return compresor.comprimir_a_bytes(mensaje)
        except Exception as e:
            print(f"Error comprimiendo Huffman: {e}")
            return None

    @staticmethod
    def descomprimir_con_huffman(mensaje_comprimido):
        """Descomprime mensaje Huffman"""
        try:
            compresor = CompresorHuffman()
            return compresor.descomprimir_desde_bytes(mensaje_comprimido)
        except Exception as e:
            print(f"Error descomprimiendo Huffman: {e}")
            return None

    @staticmethod
    def comprimir_con_zlib(mensaje):
        """Comprime mensaje usando zlib"""
        try:
            mensaje_bytes = mensaje.encode('utf-8')
            comprimido = zlib.compress(mensaje_bytes, level=9)
            return base64.b64encode(comprimido).decode('ascii')
        except Exception as e:
            print(f"Error comprimiendo zlib: {e}")
            return None

    @staticmethod
    def descomprimir_con_zlib(mensaje_comprimido):
        """Descomprime mensaje zlib"""
        try:
            comprimido = base64.b64decode(mensaje_comprimido.encode('ascii'))
            descomprimido = zlib.decompress(comprimido)
            return descomprimido.decode('utf-8')
        except Exception as e:
            print(f"Error descomprimiendo zlib: {e}")
            return None
    
    @staticmethod
    def crear_imagen_con_mensaje(mensaje, nombre_archivo="imagen_con_mensaje.png", metodo_compresion='ninguno'):
        """Crea imagen nueva con mensaje oculto"""
        try:
            ancho, alto = 200, 200
            imagen = np.random.randint(50, 200, (alto, ancho, 3), dtype=np.uint8)
            
            # Comprimir según método elegido
            print(f"\nMétodo de compresión: {metodo_compresion.upper()}")
            print(f"Tamaño original: {len(mensaje)} caracteres")
            
            if metodo_compresion == 'huffman':
                mensaje_procesado = EsteganografiaLSB.comprimir_con_huffman(mensaje)
                if mensaje_procesado is None:
                    return False
                marcador = "HUFFMAN:"
                print(f"Tamaño comprimido: {len(mensaje_procesado)} caracteres")
                print(f"Ratio: {len(mensaje)/len(mensaje_procesado):.2f}x")
                
            elif metodo_compresion == 'zlib':
                mensaje_procesado = EsteganografiaLSB.comprimir_con_zlib(mensaje)
                if mensaje_procesado is None:
                    return False
                marcador = "ZLIB:"
                print(f"Tamaño comprimido: {len(mensaje_procesado)} caracteres")
            ...
            mensaje_final = marcador + mensaje_procesado + ":END"
            mensaje_binario = ''.join(format(ord(c), '08b') for c in mensaje_final)
            
            if len(mensaje_binario) > ancho * alto:
                print(f"Mensaje demasiado largo: {len(mensaje_binario)} bits > {ancho * alto}")
                return False
    ...
    @staticmethod
    def ocultar_mensaje_en_imagen_existente(mensaje, imagen_entrada, 
                                           imagen_salida="imagen_estego.png",
                                           metodo_compresion='ninguno'):
        """
        Oculta mensaje en imagen existente con soporte para compresión
        metodo_compresion: 'ninguno', 'zlib', o 'huffman'
        """
        try:
...
def mostrar_menu():
    print("\n============================================================")
    print(" DETECTOR DE MSJS OCULTOS (LSB) - Con Huffman")
    print("============================================================")
    ...
    print("5. [NUEVO] Comparar métodos de compresión")
    print("6. Salir")
    print("============================================================")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if opcion == "1":
            ...
            print("1. Sin compresión")
            print("2. ZLIB (rápido, buen ratio)")
            print("3. HUFFMAN (óptimo para textos repetitivos)")
            
            metodo_num = input("Selecciona método (1-3, default 1): ") or "1"
            metodos_dict = {'1': 'ninguno', '2': 'zlib', '3': 'huffman'}
            metodo = metodos_dict.get(metodo_num, 'ninguno')
            
            estego.crear_imagen_con_mensaje(mensaje, f"{nombre}.png", metodo_compresion=metodo)
            input("\nPresiona Enter para continuar...")
            ...
            metodo_num = input("Selecciona método (1-3, default 2): ") or "2"
            metodos_dict = {'1': 'ninguno', '2': 'zlib', '3': 'huffman'}
            metodo = metodos_dict.get(metodo_num, 'zlib')
            
            nombre_salida = input("Archivo de salida (default: imagen_estego.png): ") or "imagen_estego.png"
            
            estego.ocultar_mensaje_en_imagen_existente(
                mensaje, 
                imgs[idx], 
                nombre_salida,
            ...
            # NUEVA OPCIÓN: Comparar métodos de compresión
            imgs = listar_imagenes()
            if not imgs:
                continue
            idx = int(input("Selecciona imagen base (numero): ")) - 1
            ...
            mensaje = input("Mensaje para prueba de compresión: ")
            
            print("\n" + "="*60)
            print("COMPARACIÓN DE MÉTODOS DE COMPRESIÓN")
            print("="*60)
            
            resultados = {}
            
            for metodo in ['ninguno', 'zlib', 'huffman']:
                print(f"\n--- Probando: {metodo.upper()} ---")
                
                archivo_temp = f"temp_{metodo}.png"
                
                inicio = time.time()
                exito = estego.ocultar_mensaje_en_imagen_existente(
                    mensaje,
                    imgs[idx],
                    archivo_temp,
                    metodo_compresion=metodo
                ...
                if exito:
                    ...
                    msg_extraido = detector.extraer_mensaje_lsb(archivo_temp)
                    
                    if msg_extraido and msg_extraido.strip() == mensaje.strip():
                        resultados[metodo] = {
                            'tiempo': tiempo,
                            'exito': True
                        }
                        ...
                        resultados[metodo] = {'exito': False}
            
            # Mostrar resumen
            print("\n" + "="*60)
            print("RESUMEN DE COMPARACIÓN")
            print("="*60)
            print(f"Mensaje original: {len(mensaje)} caracteres\n")
            
            for metodo, info in resultados.items():
                if info.get('exito'):
                    print(f"{metodo.upper():12} - Tiempo: {info['tiempo']:.4f}s")
            
            input("\nPresiona Enter para continuar...")
            
        elif opcion == "6":
            print("\n" + "="*60)
            print("Gracias por usar el Detector LSB con Huffman")
