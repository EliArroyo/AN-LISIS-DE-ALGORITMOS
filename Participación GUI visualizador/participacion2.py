...
RETARDO_MS = 80  # velocidad de animación
...
def bubble_sort_steps(data, draw_callback):
    ...
    for i in range(n-1):
        for j in range(0, n-i-1):
            draw_callback(activos=[j, j+1]); yield  # Muestra comparación
            if data[j] > data[j+1]:
                data[j], data[j+1] = data[j+1], data[j]
                draw_callback(activos=[j, j+1]); yield  # Muestra intercambio
    draw_callback(activos=[])
...
def ordenar_bubble():
    if not datos: return
    gen = bubble_sort_steps(datos.copy(), lambda activos=None: dibujar_barras(canvas, datos, activos))
    def paso():
        try:
            next(gen)
            root.after(RETARDO_MS, paso)
